...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repos_config = load_repos("repos.toml").unwrap_or_else(|e| {
        eprintln!("[ERREUR] {}", e);
        eprintln!("Vérifiez que repos.toml existe et est valide.");
        std::process::exit(1);
    });

    if repos_config.repos.is_empty() {
        log::warn!("Aucun dépôt configuré dans repos.toml");
    }

    log::info!("=== GitHub Tracker démarré ===");
    log::info!(
        "Surveillance de {} dépôt(s)",
        repos_config.repos.len()
    );
    log::info!(
        "Intervalle de vérification : {}s",
        app_config.check_interval_secs
    );

    let shared_state: Arc<Mutex<AppState>> = Arc::new(Mutex::new(AppState::new()));
    ...
    // Spawn the monitoring loop in a background task
    {
        ...
        let repos = repos_config.repos.clone();
        ...
        if let Some(prev) = &prev_state {
            ...
            if repo_cfg.notify_stars && gh_repo.stargazers_count != prev.stars {
                ...
                new_notifications.push(Notification {
                    ...
                    message: format!(
                        "Stars : {} → {} ({:+})",
                        prev.stars, gh_repo.stargazers_count, diff
                    ),
                    timestamp: Utc::now(),
            ...
            if repo_cfg.notify_forks && gh_repo.forks_count != prev.forks {
                ...
                new_notifications.push(Notification {
                    ...
                    message: format!(
                        "Forks : {} → {} ({:+})",
                        prev.forks, gh_repo.forks_count, diff
                    ),
                    timestamp: Utc::now(),
            ...
            if repo_cfg.notify_releases {
                if let Some(new_tag) = &latest_release_tag {
                    let is_new = prev
                        .latest_release
                        .as_ref()
                        .map_or(true, |old| old != new_tag);
                    if is_new {
                        let release = latest_release.as_ref().unwrap();
                        let tg_msg = format!(
                            "🚀 <b>{name}</b>\nNouvelle release : <b>{tag}</b>\n<a href=\"{url}\">{url}</a>",
                            name = full_name,
                            tag = new_tag,
            ...
            // First pass — record without notifying
            log::info!(
